...
if [ .$system_branch = .'master' ]; then
	...
	system_major=$(git ls-remote --heads https://github.com/samael33/fusionpbx.git | cut -d/ -f 3 | grep -P '^\d+\.\d+$' | sort | tail -n 1 | cut -d. -f1)
	system_minor=$(git ls-remote --tags https://github.com/samael33/fusionpbx.git $system_major.* | cut -d/ -f3 |  grep -P '^\d+\.\d+$' | sort | tail -n 1 | cut -d. -f2)
